...
export default class DiscordEvent {
    ...
    set startDatetime(value: string | Date) {
        ...
        if (this._endDateIsSet && this._endTimeIsSet && datetime.getTime() >= this.endDatetime.getTime()) {
    ...
    set startDate(value: string | Date) {
        ...
        if (this._startTimeIsSet && date.getTime() + this.startTime.getTime() <= Date.now()) {
            throw new E.EventDatePastError;
        }
        // if the end date/time and start time are alredy set, check if the new start date/time would lie after the end date/time
        if (this._endDateIsSet && this._endTimeIsSet && this._startTimeIsSet && date.getTime() >= this._endDatetime.getTime()) {
            throw new E.EventEndsBeforeStartError;
        }

        this.startDatetime.setFullYear(date.getFullYear(), date.getMonth(), date.getDate());
        this._startDateIsSet = true;
    }
    get startDate(): Date {
    ...
    set startTime(value: string | Date) {
        ...
        if (this._startDateIsSet && time.getTime() + this.startDatetime.getTime() <= Date.now()) {
            throw new E.EventDatePastError;
        }
        // if the end date/time and start date are alredy set, check if the new start time would lie after the end date/time
        if (this._endDateIsSet && this._endTimeIsSet && this._startDateIsSet && time.getTime() + this.startDate.getTime() >= this.endDatetime.getTime()) {
            throw new E.EventEndsBeforeStartError;
        }
        this._startDatetime.setHours(time.getHours(), time.getMinutes(), time.getSeconds());
        this._startTimeIsSet = true;
    }
    get startTime(): Date {
    ...
    set endDatetime(value: string | Date) {
        ...
        if (this._startDateIsSet && this._startTimeIsSet && datetime.getTime() <= this.startDatetime.getTime()) {
            throw new E.EventEndsBeforeStartError;
        }
        if (datetime.getTime() <= Date.now()) {
            throw new E.EventDatePastError;
    ...
    set endDate(value: string | Date) {
        ...
        if (this._startDateIsSet && this._startTimeIsSet && this._endTimeIsSet && date.getTime() + this.endTime.getTime() <= this.startDatetime.getTime()) {
            throw new E.EventEndsBeforeStartError;
        }
        // if the time is already set, check if the new date would lie in the past
        if (this._endTimeIsSet && date.getTime() + this.endTime.getTime() <= Date.now()) {
            throw new E.EventDatePastError;
        }

        this.endDatetime.setFullYear(date.getFullYear(), date.getMonth(), date.getDate());
        this._endDateIsSet = true;
    }
    get endDate(): Date {
    ...
    set endTime(value: string | Date) {
        ...
        // if the start date/time and end time are alredy set, check if the new end date would lie before the start date/time
        if (this._startDateIsSet && this._startTimeIsSet && this._endDateIsSet && time.getTime() + this.endDate.getTime() <= this.startDatetime.getTime()) {
            throw new E.EventEndsBeforeStartError;
        }
        // if the date is already set, check if the new time would lie in the past
        if (this._endDateIsSet && time.getTime() + this._endDatetime.getTime() <= Date.now()) {
            throw new E.EventDatePastError;
        }

        this._endDatetime.setHours(time.getHours(), time.getMinutes(), time.getSeconds());
        this._endTimeIsSet = true;
    }
    get endTime(): Date {
